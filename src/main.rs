...
impl Display for Semester {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Spring(y) => write!(f, "Spring {}", y),
            Self::Summer(y) => write!(f, "Summer {}", y),
            Self::Fall(y) => write!(f, "Fall {}", y),
        }
    }
}
...
pub fn get_calendar(path: &str) -> Result<Vec<Entry>, Box<dyn Error>> {
...
pub fn generate_ics(path: &str) -> Result<String, Box<dyn Error>> {
    let mut calendar = ICalendar::new("2.0", "icalendar");

    let entries = get_calendar(path)?;
    ...
    for entry in entries {
        ...
        let mut event = Event::new(format!("{:x}", ev_hash), Utc::now().format("%Y%m%dT000000").to_string());
    ...
    calendar.save_file("calendar.ics")?;
    Ok(calendar.to_string())
}

pub fn main() {
    let path = std::env::args().nth(1).expect("path not provided");
    let cal = generate_ics(path.as_str()).expect("could not generate calendar");
    println!("{}", cal);
}
